...
var paths = {
  ...
  find_replace: {
    ...
    replace_live_ambassadors:
      'https://ambassadors.thedonut.co/wp-content/themes/theDONUT',
  },
};
...
var nunjucks = function (done) {
  return src(paths.nunjucks.input)
    .pipe(
      gulpData(function () {
        return JSON.parse(
          fs.readFileSync(`./${paths.input}nunjucks/data.json`)
        );
      })
    )
    .pipe(
...
var copyCSS = function (done) {
  ...
  // Copy CSS files
  return src(paths.copy.css.input).pipe(dest(paths.copy.css.output));
};
...
var copyJS = function (done) {
  ...
  // Copy JS files
  return src(paths.copy.js.input).pipe(dest(paths.copy.js.output));
};
...
var copyFonts = function (done) {
  ...
  // Copy Font files
  return src(paths.copy.fonts.input).pipe(dest(paths.copy.fonts.output));
};
...
var copyImages = function (done) {
  ...
  // Copy Image files
  return src(paths.copy.images.input).pipe(dest(paths.copy.images.output));
};
...
var copyRootFiles = function (done) {
  ...
  // Copy Image files
  return src(paths.copy.root.input).pipe(dest(paths.copy.root.output));
};
...
function replaceLiveAmbassadors() {
  return src(paths.find_replace.input)
    .pipe(
      replace(
        paths.find_replace.find,
        paths.find_replace.replace_live_ambassadors
      )
    )
    .pipe(dest(paths.copy.root.output));
}
...
// Copy Task
// gulp copy
exports.copy = series(copyCSS, copyJS, copyFonts, copyImages, copyRootFiles);
...
exports.default = series(
  cleanDist,
  nunjucks,
  parallel(buildScripts, lintScripts, buildStyles, buildSVGs, exports.copy)
);
